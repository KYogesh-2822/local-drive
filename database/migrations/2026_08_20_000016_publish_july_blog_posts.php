...
return new class extends Migration
{
    private const DATA_FILES = [
        'common-airport-car-rental-mistakes-and-how-to-avoid-them.json',
        'rental-car-guide-suv-vs-sedan-for-travel-in-jordan.json',
        'car-hire-guide-private-driver-tips-for-travel-in-jordan.json',
    ];
    ...
    private function loadBlogData(): array
    {
        return collect(self::DATA_FILES)->map(function (string $file): array {
            $path = database_path('data/2026-07-blogs/'.$file);

            if (! is_file($path)) {
                throw new RuntimeException("Missing July blog data file: {$path}");
            }

            $data = json_decode((string) file_get_contents($path), true, 512, JSON_THROW_ON_ERROR);

            if (($data['status'] ?? null) !== 'published' || count($data['faqs'] ?? []) !== 5) {
                throw new RuntimeException("Invalid July blog data file: {$path}");
            }
    ...
    private function validateFeaturedImages(array $blogs): void
    {
        foreach ($blogs as $blog) {
            ...
            if (! is_file($path)) {
                throw new RuntimeException("Missing July blog featured image: {$path}");
            }
        }
    }
